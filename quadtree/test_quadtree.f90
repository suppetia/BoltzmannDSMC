program testQuadTree
  ...
  use m_quadtree, only: QuadTree, QuadTreeNode, splitNode, deleteSubTree, initializeQuadTree, &
  ...
  use m_matrix_io, only: readIntMatrixFromFile, writeRealMatrixToFile
  implicit none
  ...
  call initializeQuadTree(tree, 15, 5, real(size(matrix, 2), dp), real(size(matrix, 1), dp))
  print *, tree%root%numberOfElements
  call buildTreeFromMatrix(tree, matrix)
  print *, tree%root%numberOfElements
  
  allocate(cellMatrix(100000, 4*(1 + tree%maxElementsPerCell)))
  ...
  call writeRealMatrixToFile(filenameCells, cellMatrix, j, 4*(1+tree%maxElementsPerCell), status)
